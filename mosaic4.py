...
def is_valid_raster(src):
    try:
        ...
        if src.transform.a == 0 or src.transform.e == 0:
            return False
        if np.any(np.isnan(src.read(1, out_shape=(1, 1)))):
            return False
        return True
...
def stitch_geotiffs_incremental(input_folder, output_path, output_crs=None, keep_temp=False):
    ...
    blending overlapping edges using average merge method.

    Args:
        input_folder (str): Path to folder containing .tif files
        output_path (str): Path to output mosaic .tif
        output_crs (str): Optional CRS to reproject all rasters to
        keep_temp (bool): If True, keep intermediate temp files for debugging
    """
    tiff_files = glob.glob(os.path.join(input_folder, '*.tif'))
    if not tiff_files:
        raise FileNotFoundError("No .tif files found in the input directory.")

    tiff_files.sort()  # Ensure consistent order
    valid_sources = []
    ...
    logging.info("Blending all rasters with full overlap handling using 'max' method...")

    try:
        mosaic, transform = merge(valid_sources, method='max')
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Stitch GeoTIFFs into a mosaic with blending overlaps.")
